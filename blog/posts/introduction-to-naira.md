...
Each of these tools may be good at its own job. But the full picture is hard to see. The ground is given for shadow AI.
...
AI engineering needs more than another specialized tool.
...
Naira is not just a platform, a portal or a catalog. It is maybe something like an internal development platform (IDP), but we aim for more than that.
...
So, it is a **hub**. A hub connects things. It creates orientation. It helps users move between systems. It can start by making existing tools visible and understandable, then grow into a place where approved actions and workflows can be initiated safely.
...
Plugins allow teams and communities to connect Naira to additional systems. But plugins must not make the user experience inconsistent or confusing. Naira therefore needs clear expectations for plugin metadata, documentation, UI extension points, lifecycle states, ownership, and trust.
...
Naira solves the fragmentation problem by some principles and ideas. They may change over time and be adjusted to enduser feedback. But for now they help us to get the direction.
...
AI engineering and the entire "AI" domain is continously evolvoing, partially with a pace where one finding of last week, gets overruled by a new finding this week. The tooling landscape is changing quickly and organizations have different requirements around infrastructure, compliance, data, providers and operational models. A massive challenge for companies.
